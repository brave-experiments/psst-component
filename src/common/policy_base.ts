...
export abstract class PolicyScriptBase {
    ...
    abstract waitForSettingAppliedWithTimeout(selector: string | undefined, turn_off: boolean | undefined): Promise<void>;

    async applyPolicies(): Promise<PolicyScriptResult> {
        if (__DEV__) logger.info('Starting applyPolicies with params:', this.params);
        ...
        try {
            ...
            await this.waitForSettingAppliedWithTimeout(current_task?.selector, current_task?.turn_off)
    ...
    private parseParams(): PolicyScriptInputData {
        ...
        // reads `params.tasks` directly). It is NOT a property of window/globalThis,
        // so we must reference the bare identifier and resolve it via the scope
        // chain. Guard with `typeof` so a missing binding yields a fallback
        ...
        || '{}';
        if (__DEV__) logger.info('Parsing PolicyScriptInputData from params:', rawParams);
